...
class WaitTimeReport:
    ...
    def table11_part2_report_on_wait_time(self):
        ...
        wait_time_stats_by_stage_phase = wait_time_stats_by_stage_phase.sort_values(by=['Stage', 'mean'], ascending=[True, False])
    ...
    def table11_part3_report_on_wait_time(self):
        def count_non_zero(x):
            return (x > 0).sum()
        wait_time_stats_by_stage_size = self.phases_df.groupby(['Stage', 'Project Scale'])['Wait Time to Obtain Resources'].agg(['count', 'mean', 'min', 'max', count_non_zero])
        wait_time_stats_by_stage_size = wait_time_stats_by_stage_size.rename(columns={"count_non_zero": "Count > 0"})
        wait_time_stats_by_stage_size = wait_time_stats_by_stage_size.sort_values(by=['Stage', 'Project Scale', 'mean'], ascending=[True, True, False])
    ...
    def table11_part4_report_on_wait_time(self):
        def count_non_zero(x):
            return (x > 0).sum()
        wait_time_stats_by_stage = self.phases_df.groupby(['Stage'])['Wait Time to Obtain Resources'].agg(['count', 'mean', 'min', 'max', count_non_zero])
        wait_time_stats_by_stage = wait_time_stats_by_stage.rename(columns={"count_non_zero": "Count > 0"})
        wait_time_stats_by_stage = wait_time_stats_by_stage.sort_values(by=['Stage', 'mean'], ascending=[True, False])
